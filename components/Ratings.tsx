...
const RatingDistributionChart = ({ data }: { data: RatingItem[] }) => {
  ...
  const counts = useMemo(() => {
    const c = new Array<number>(11).fill(0);
    let max = 0;
    data.forEach(d => {
...
const Ratings: React.FC = () => {
  ...
  useEffect(() => {
    fetch(csvUrl)
      ...
      .then((text) => {
        ...
        setData(parsedData);
        // Pick spotlight from high-rated items once data loads
        const highRated = parsedData.filter(i => i.myRating >= 8);
        const candidates = highRated.length > 0 ? highRated : parsedData;
  ...
  // Reset pagination when filters change (render-time adjustment, avoids effect cascade)
  const [prevFilters, setPrevFilters] = useState({ searchTerm, selectedType, sortConfig });
  if (
    prevFilters.searchTerm !== searchTerm ||
    prevFilters.selectedType !== selectedType ||
    prevFilters.sortConfig !== sortConfig
  ) {
    setPrevFilters({ searchTerm, selectedType, sortConfig });
    setVisibleCount(60);
  }
          ...
          >
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full pb-6 md:pb-16">
              <div className="flex flex-col md:flex-row items-start md:items-end gap-4 md:gap-8">
                ...
                >
                  <img src={spotlightItem.pictureUrl} alt={spotlightItem.title} className="w-full h-full object-cover" />
                </motion.div>
